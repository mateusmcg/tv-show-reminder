                    ...
                    return SearchRestService.getList({query: search, type:'movie'}).then(function(movies){
                        ...
                        return SearchRestService.getList({query: search, type:'person'}).then(function(people){
                            ...
                            angular.forEach(partialResults, function(value, index){
                                if(value.type === 'show'){
                                    var showImages = value.show.images.poster;
                                    var showImg = showImages.thumb ? showImages.thumb : showImages.medium ? showImages.medium : showImages.full ? showImages.full : null;
                                    var showYear = value.show.year ? ' (' + value.show.year + ')' : '';
                                    results.push({name: value.show.title + showYear, img: showImg, group: 'Shows', _id: value.show.ids.trakt, trackBy: 'Shows-' + value.show.ids.trakt});
                                } else if(value.type === 'movie'){
                                    var movieImages = value.movie.images.poster;
                                    var movieImg = movieImages.thumb ? movieImages.thumb : movieImages.medium ? movieImages.medium : movieImages.full ? movieImages.full : null;
                                    var movieYear = value.movie.year ? ' (' + value.movie.year + ')' : '';
                                    results.push({name: value.movie.title + movieYear, img: movieImg, group: 'Movies', _id: value.movie.trakt, trackBy: 'Movies-' + value.movie.trakt});
                                } else{
                                    var personImages = value.person.images.headshot;
                                    var personImg = personImages.thumb ? personImages.thumb : personImages.medium ? personImages.medium : personImages.full ? personImages.full : null;
                                    results.push({name: value.person.name, img: personImg, group: 'People', _id: value.person.trakt, trackBy: 'People-' + value.person.trakt});
                                }
                            });
